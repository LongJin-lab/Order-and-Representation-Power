...
from modelsMDP import SegDecNet
...
class End2End:
    ...
    def training_iteration(self, data, device, model, criterion_seg, criterion_dec, optimizer, weight_loss_seg, weight_loss_dec,
        ...
        for sub_iter in range(num_subiters):
            ...
            noise_type = 'randn'
            if self.cfg.TRAIN_NOI_COE:
                # In-place un-normalization
                # input.mul_(IMAGENET_DEFAULT_STD).add_(IMAGENET_DEFAULT_MEAN)
                # print('torch.max(images_), torch.min(images_)', torch.max(images_), torch.min(images_))
                if noise_type == 'randn':
                    images_.add_(self.cfg.TRAIN_NOI_COE * torch.randn_like(images_).to(device))
                elif noise_type == 'rand':
                    images_.add_(self.cfg.TRAIN_NOI_COE * torch.rand_like(images_).to(device))
                elif noise_type == 'const':
                    images_.add_(self.cfg.TRAIN_NOI_COE)            
            ...
            if epoch % 5 == 0:
                ...
                for name, p in model.named_parameters():
                    if 1:#p.requires_grad: 
                        if name:
                            if 'd_alpha' in name: # and 'bn3' not in name:
                                # print(name)
                                # sum_A = sum_A+p
                                # coes_As = coes_As+[p]
                            ...
                            elif 'd_beta' in name: # and 'bn3' not in name:
                                # print(name)
                                # sum_B = sum_B+p
    ...
    def eval_model(self, device, model, eval_loader, save_folder, save_images, is_validation, plot_seg):
        ...
        for data_point in eval_loader:
            ...
            if self.cfg.TRAIN_NOI_COE:
                ...
                # print('torch.max(images_), torch.min(images_)', torch.max(images_), torch.min(images_))
                if noise_type == 'randn':
                    image.add_(self.cfg.TRAIN_NOI_COE * torch.randn_like(image).to(device))
                elif noise_type == 'rand':
                    image.add_(self.cfg.TRAIN_NOI_COE * torch.rand_like(image).to(device))
                elif noise_type == 'const':
                    image.add_(self.cfg.TRAIN_NOI_COE)   
