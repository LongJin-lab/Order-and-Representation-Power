...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.conv1 = _conv_block(in_channels, out_channels, kernel_size, stride)
        # self.conv2 = _conv_block(out_channels, out_channels, kernel_size, stride)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + residual
        return out    
# Define a custom update block
class CustomUpdateBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, alpha, d_alpha, beta, d_beta, tau=1):
        super(CustomUpdateBlock, self).__init__()
        self.conv = _conv_block(in_channels, out_channels, kernel_size, padding)
        self.alpha = alpha
        # self.d_alpha = d_alpha
        self.beta = beta
        # self.d_beta = d_beta
        self.tau = 0.1
        trainA, trainB = True, True
        # trainA, trainB = False, False        
        self.d_alpha = nn.Parameter(torch.zeros_like(torch.tensor(self.alpha)), requires_grad=trainA)
        self.d_beta = nn.Parameter(torch.zeros_like(torch.tensor(self.beta)), requires_grad=trainB)        
    def forward(self, x, prev_states, prev_fs):
        f_x = self.conv(x)
        prev_states.append(x)
        prev_fs.append(f_x)
        
        # Update x using the custom equation
        if len(prev_states) >= len(self.alpha) and len(prev_fs) >= len(self.beta):
            x_new = sum((-self.alpha[i] - 0.1*self.d_alpha[i]) * prev_states[-i-1] for i in range(len(self.alpha))) + \
                    self.tau*sum((self.beta[i] + 1000.0*self.d_beta[i]) * prev_fs[-i-1] for i in range(len(self.beta)))
            # print('sum')
            # Remove the oldest state and f(x) if the lists are too long
            if len(prev_states) > len(self.alpha):
                prev_states.pop(0)
            if len(prev_fs) > len(self.beta):
                prev_fs.pop(0)
                
            return x_new, prev_states, prev_fs
        else:
            # print('res')
            x_new = x + self.tau * f_x
            return x, prev_states, prev_fs
...
class SegDecNet(nn.Module):
    def __init__(self, device, input_width, input_height, input_channels):
        ...
        self.beta = [0., 1., 0., 0.]  
        self.d_alpha = self.alpha
        self.d_beta = self.beta
        # trainA, trainB = True, True
        ...
        #                             nn.MaxPool2d(2),
        #                             _conv_block(64, 1024, 15, 7))
        self.volume = nn.Sequential(_conv_block(input_channels, 32, 5, 2),
                                    # _conv_block(32, 32, 5, 2), # Has been accidentally left out and remained the same since then
                                    nn.MaxPool2d(2),
                                    _conv_block(32, 64, 5, 2),
                                    _conv_block(64, 64, 5, 2),
                                    nn.MaxPool2d(2),
                                    # CustomUpdateBlock(32, 64, 5, 2, self.alpha, self.d_alpha, self.beta, self.d_beta),                                    
                                    # CustomUpdateBlock(64, 64, 5, 2, self.alpha, self.d_alpha, self.beta, self.d_beta),
                                    CustomUpdateBlock(64, 64, 5, 2, self.alpha, self.d_alpha, self.beta, self.d_beta),
                                    # nn.MaxPool2d(2),
                                    CustomUpdateBlock(64, 64, 5, 2, self.alpha, self.d_alpha, self.beta, self.d_beta),
                                    CustomUpdateBlock(64, 64, 5, 2, self.alpha, self.d_alpha, self.beta, self.d_beta),
    ...
    def forward(self, x):
        ...
        for layer in self.volume:
            if isinstance(layer, CustomUpdateBlock):
                x, prev_states, prev_fs = layer(x, prev_states, prev_fs)
            else:
                x = layer(x)        
        seg_mask = self.seg_mask(x)
        cat = torch.cat([x, seg_mask], dim=1)
