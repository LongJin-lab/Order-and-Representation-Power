            ...
            if 'ConvStride2Learn' in self.ablation:
                ...
                for i in range(self.steps-1):
                    ...
                    if pre_acts[i].shape[1] != F_x_n.shape[1]:
                        pre_acts[i] = self.LearnDS[i](pre_acts[i])
            else:
                if self.in_planes != self.planes:
                    residual = self.downsample_x_Fix(residual)  
                for i in range(self.steps-1):
...
if __name__ == '__main__':
    ...
    d = torch.rand(1, 3, 4, 4)
    print('d, d.shape', d, d.shape)
    out00 = F.pad(d, pad=(0,0,0,0), mode='constant', value=0)
    out01 = F.pad(d, pad=(-1,1,0,0), mode='constant', value=0)
    out10 = F.pad(d, pad=(0,0,-1,1), mode='constant', value=0)
    out11 = F.pad(d, pad=(-1,1,-1,1), mode='constant', value=0)
    ...
    print('out00, out01, out10, out11', out00, out01, out10, out11)
    print('out00.shape', out00.shape)
    out = torch.cat((out00, out01, out10, out11), dim=1)
