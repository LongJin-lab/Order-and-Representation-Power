    ...
    def forward(self, x, pre_features, pre_acts, coesA, coesB, coes_stepsize):  
        ...
        if not (self.has_ini_block):
            ...
            if 'ExpDecay' in self.Settings:
                residual_ExpDecay = torch.exp(self.coesDecay*self.Layer_idx*coes_stepsize)*(residual-self.bias_inner)
            else:
                residual_ExpDecay = residual
            if 'DSfirst' in self.Settings:      
                # print("==>> type(DSfirst): ")                 
                F_x_n = self.bn1(residual_ExpDecay)
                F_x_n = self.relu(F_x_n)
                F_x_n = self.conv1(F_x_n)
                if self.drop_rate:
                    F_x_n = F.dropout(F_x_n, p=float(self.drop_rate), training=self.training)               
            ...
            if 'ExpDecay' in self.Settings:
                # print('self.coesDecay',self.coesDecay)
                F_x_n = torch.exp(-self.coesDecay*self.Layer_idx*coes_stepsize) * F_x_n -self.coesDecay*residual+self.bias_outer
    ...
    def __init__(self, block, layers, coe_ini=1, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], share_coe=False, ini_stepsize=1, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        if not self.stochastic_depth:
            for i in range(3):
                ...
                print('l',l)
                blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB,coes_stepsize=self.ini_stepsize, steps=self.steps, stride=self.strides[i], coe_ini=self.coe_ini, Settings=self.Settings,Layer_idx=Layer_idx, coesDecay=self.coesDecay))
                ...
                for k in range(1, layers[i]):
                    ...
                    blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB, coes_stepsize=self.ini_stepsize, steps=self.steps, coe_ini=self.coe_ini, Settings=self.Settings,Layer_idx=Layer_idx, coesDecay=self.coesDecay))
...
# ], Settings='mnistAllElePlotTraj')#S4O2 ExpDecay
    ...
    net = ZeroSAny32Settings(givenA=[1,0
], givenB=[-1,0
], Settings='2Chs_BnReluConv_ConvStride2ResLike_ShareExpDecayLearnDecay_AbsExp_Adam_LREndBN_RestaLayerIdx')#S4O4
